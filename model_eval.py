...
def plot(pred, label, lon, lat, i, mode):
    ...
    pred[pred > 150] = 150
    label[label > 150] = 150
    cf1 = plt.contourf(lon, lat, pred, transform=ccrs.PlateCarree(), levels=range(-50, 50))
    ...
    cf2 = plt.contourf(lon, lat, label, transform=ccrs.PlateCarree(), levels=range(-50, 50))
...
def eval_ts():
    ...
    weight = np.load('weight.npy')
    weight = weight.reshape([-1, 14])
    cor_list, pred_station, label_station, diff_station, aqms_station = [], [], [], [], []
    # station = [150, 150]
    # station = [78, 182]
    station = [79, 168]
    # station = [120, 154]
    ...
    if lag:
        plt.plot(x, pred_station[lag:], 'b', x, label_station[:-lag], 'r')#, x, diff_station[:-lag], 'k')
    else:
        plt.plot(x, pred_station, 'b', x, label_station, 'r')#, x, diff_station, 'k')
    plt.show()
